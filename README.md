...
**Denominator.** The five populations' counts sum to a sample's total; every percentage in the dashboard is a cell frequency, count divided by that sum, and the charts label it "Cell frequency (%)".
...
| A selectable denominator                       | One fixed denominator, the sum of the five populations; the axis reads "Cell frequency (%)"                                          | The dataset has only five populations; the label states exactly what is being divided                                                    |
